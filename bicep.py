...
def gen_frames(d):
    ...
    def calculate_angle(a,b,c):
        ...
        radians = np.arctan2(c[1]-b[1], c[0]-b[0]) - np.arctan2(a[1]-b[1], a[0]-b[0]) #(y2-y1)(x2-x1)
        angle = np.abs(radians*180.0/np.pi) #conversion into degree
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        while cap.isOpened():
            ret, frame = cap.read() #30-40 frames per second
